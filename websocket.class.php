...
class WebSocket{
  ...
  function __construct($address,$port){
	...
	$this->ajax=socket_create(AF_INET, SOCK_STREAM, SOL_TCP)     or die("socket_create() failed");
    socket_set_option($this->ajax, SOL_SOCKET, SO_REUSEADDR, 1)  or die("socket_option() failed");
    socket_bind($this->ajax, $address, $port+1)                    or die("socket_bind() failed");
    socket_listen($this->ajax,20)                                or die("socket_listen() failed");
	
    $this->sockets[] = $this->master;
	$this->sockets[] = $this->ajax;
    $this->say("Server Started : ".date('Y-m-d H:i:s'));
    $this->say("Listening on   : ".$address." port ".$port);
    $this->say("Master socket  : ".$this->master);
	$this->say("Ajax socket  : ".$this->ajax."\n");
    ...
    while(true){
      ...
      foreach($changed as $socket){
		...
		else if($socket==$this->ajax){
          $client=socket_accept($this->ajax);
          if($client<0){ $this->log("socket_accept() failed"); continue; }
          else{ 
			$this->log($socket." Connecting");
			$this->connectajax($client); 
  ...
  function connect($socket){
    ...
    function connectajax($socket){ //Ajax version without handshake.
    $user = new User();
    $user->id = uniqid();
    $user->socket = $socket;
	$user->handshake = true;
